...
//  std::string frq = pQA->qaExp.getFrequency();
  ...
  Split x_fName;
  x_fName.enableEmptyItems();
  x_fName.setSeparator('_');
  x_fName = fName ;

  // find 8xdigits[-8xdigits]
  int startTime_EndTime_ix = -1;
  std::string fName_T;
  ...
  testPeriodFormat(x_fName, sd) ;

  // check whether item 'Reference_period' was found. If yes, adjust
  if( sd[0].size() == 4 && sd[1].size() == 4)
  {
      ...
      size_t p0;
      if( (p0=fName.find(s0)) < std::string::npos )
        fName_T = fName.substr(0,p0);
  ...
  for( size_t i=0 ; i < x_fName.size() ; ++i )
  {
      if( x_fName[i].substr(0,8) == sd[0] )
      {
        startTime_EndTime_ix = i;
        break;
      }
  }

  checkFilenameEncoding(fName, drs_cv_table, startTime_EndTime_ix) ;
...
void
DRS_CV::checkFilenameEncoding(std::string& fName, struct DRS_CV_Table& drs_cv_table
    , int startTime_EndTime_ix)
{
  ...
  // number of occurrences of filename components in global attributes
  std::vector<size_t> count_occ(enc_sz, 0);

  // dito for drs key matching corresponding att name
  std::vector<size_t> count_drs_compat(enc_sz, 0);

  // collection of drs items , which are not within global atts
  std::vector<std::vector<std::string> > failed_drs_item;
  std::vector<std::vector<std::string> > failed_fName_item;

  // number of items in model-derived DRS [0] and general DRS [1], respectively.
  std::vector<size_t> sz_xfName(enc_sz, 0);
...
//  for( size_t ds=0 ; ds < enc_sz ; ++ds)
  ...
  {
    // note that the separator could change
    x_fName.setSeparator(drs_cv_table.fNameSep[ds]);
    x_fName = fName ;

    failed_fName_item.push_back(std::vector<std::string>()) ;

    Split& x_dir = x_enc[ds] ;  // DRS Filename facets
    x_dir.setSeparator(drs_cv_table.fNameSep[ds]);
    ...
    if( drs_cv_table.fNameEncoding[ds].rfind(".nc") < std::string::npos )
    {
      ...
      if( t.substr(t.size()-3) == ".nc" )
        x_dir = t.substr(0, t.size()-3) ;
    }
    else
      x_dir = drs_cv_table.fNameEncoding[ds] ;

    sz_xfName[ds] = x_fName.size();

    isFNameReq.push_back(std::vector<bool>());
    for(size_t ix=0 ; ix < x_fName.size() ; ++ix )
    {
        // find filename items also present in the glob atts
        bool is=false;
        size_t index_0;
        size_t index_1;
        if( ix == 0 )
        {
            if( hdhC::isAmong(x_fName[ix], pQA->pIn->nc.layout.varName) )
                // var name is usually not present in global atts
                ++count_occ[ds];
            else
            {
                if( notes->inq( "1_4", "DRS") )
                  (void) notes->operate("Variable name ambiguously in filename and file") ;
                ++count_occ[ds];
            }
        }
        else if( hdhC::isAmong(x_fName[ix], globalVar.attValue, index_0, index_1) )
        {
          ++count_occ[ds];
          is = true;
        }
        else if( startTime_EndTime_ix == static_cast<int>(ix) )
        {
            ++count_occ[ds];
            ++count_drs_compat[ds];
        }
        else
            failed_fName_item[ds].push_back(x_fName[ix]);

        if(is)
        {
            // try for a DRS <--> CV relation
            std::string aName;

            if(ix)
                aName = globalVar.attName[index_0];

            if(ix==0)
                ++count_drs_compat[ds];
            else if(cvMap.count(aName) > 0 )
                ++count_drs_compat[ds];
        }
    }
  }
  ...
  if( count_occ[0] < static_cast<size_t>(5) )
  {
    capt = "Type of impact indicator DRS not diagnosable, lack of required items";
    key = "CLIPC_0ca";
  }
  else if( count_occ[0] > static_cast<size_t>(8) )
  {
    capt = "Possibly MODEL-derived impact indicator DRS.";
    key = "CLIPC_0a";

    if( failed_fName_item[0].size() )
        missing = hdhC::getVector2Str(failed_fName_item[0]);
  }
  else if( count_occ[0] < static_cast<size_t>(9) )
  {
    capt = "Possibly GENERAL impact indicator DRS.";
    key = "CLIPC_0b";

    if( failed_fName_item[1].size() )
        missing = hdhC::getVector2Str(failed_fName_item[0]);
  }
  else
  {
    capt = "Type of impact indicator DRS not diagnosable";
    key = "CLIPC_0ca";
  }

  if( capt.size() &&  notes->inq( key, "DRS") )
     (void) notes->operate(capt) ;

  if( missing.size() &&  notes->inq( "CLIPC_1", "DRS") )
  {
     capt = "filename items missing in files meta-data: ";
     capt += missing ;
     (void) notes->operate(capt) ;
  }
...
DRS_CV::testPeriod(Split& x_f)
{
  ...
  if( testPeriodFormat(x_f, sd) )
  {
    if( sd[0].size() )
    {
      ...
      Date tDate(pQA->qaTime.refDate);
      if( pQA->qaTime.firstTimeValue != 0. )
        tDate.addTime(pQA->qaTime.firstTimeValue);
      ...
      if( tDate != period[0] )
      {
        ...
        {
            std::string capt("Misaligned time: found ");

            capt += hdhC::tf_assign("StartTime", period[0].str()) ;
            capt += " and ";
            capt += hdhC::tf_assign("time value", tDate.str() );
...
{
  // only test for [ filenme_range ], i.e. equal or within
  ...
  double lDiff[]={0., 0., 0., 0.};
  double rDiff[]={0., 0., 0., 0.};

  Date myDate[2];

  // time value: left-side
  myDate[0] = *pDates[2] ;
  myDate[0].addTime(-pQA->qaTime.refTimeStep/2.);
  lDiff[0] = myDate[0] - *pDates[0] ;  // positive
  rDiff[0] = *pDates[1] - myDate[0] ;  // positive

  // time value: right-side
  myDate[1] = *pDates[3] ;
  myDate[1].addTime(pQA->qaTime.refTimeStep/2.);
  lDiff[1] = myDate[1] - *pDates[0] ;  // positive
  rDiff[1] = *pDates[1] - myDate[1] ;  // positive

  if(pQA->qaTime.isTimeBounds)
  {
    // note: value of a younger date is higher
    // time_bounds: left-side
    lDiff[2] = *pDates[4] - *pDates[0] ;  // positive ok
    rDiff[2] = *pDates[1] - *pDates[4] ;  // positive

    // time_bounds: right-side
    lDiff[3] = *pDates[5] - *pDates[0] ;  // positive
    rDiff[3] = *pDates[1] - *pDates[5] ;  // positive
  }
  ...
  for(size_t i=0 ; i < 4 ; ++i)
  {
    ...
    if( lDiff[i] < 0. || rDiff[i] < 0. )
    {
      int ll = i % 2 ;  // left or right side

      std::string key("1_6g");
      if( notes->inq( key, pQA->fileStr) )
      {
        std::string capt("Misaligned times: found ");

        if( ll == 0 )  // left side
        {
          capt += hdhC::tf_assign("StartTime", pDates[0]->str()) ;
          if( i > 2 )
             capt += hdhC::tf_assign("time bounds", pDates[4]->str() );
          else
             capt += hdhC::tf_assign("time value", pDates[2]->str() );
        }
        else
        {
          capt += hdhC::tf_assign("EndTime", pDates[1]->str()) ;
          if( i > 2 )
             capt += hdhC::tf_assign("time bounds", pDates[5]->str() );
          else
             capt += hdhC::tf_assign("time value", pDates[3]->str() );
        }
